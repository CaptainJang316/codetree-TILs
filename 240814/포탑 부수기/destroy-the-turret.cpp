// 11:37 ~ 1:45, 4:50 ~ 6:10, 10:15 ~ 11:20, 1:35 ~ 2:00, 11:20 ~ 
// 
// 
...
void laserWayDFS(int r, int c, int depth) {
    ...
    for (int d = 0; d < 4; d++)
    {
        ...
        //if (nr < 1) nr = N;
        //else if (nr > N) nr = 1;
        //else if (nc < 1) nc = M; <-- 와 이런 실수를;;;;; 다 else if으로 해버리니, nr, nc가 둘다 범위 나갈 경우가 고려되지 못했다;;;;
        //else if (nc > M) nc = 1;
        if (nr < 1) nr = N;
        else if (nr > N) nr = 1;
        if (nc < 1) nc = M;
...
int main() {
    ...
    for (int k = 1; k <= K; k++)
    {
        ...
        // 공격자 선정
        //int a = 0; <--- 위의 전역변수로 선언 변경 해놓고 여기도 선언한 거 안지워서 문제됨;;;
        a = 0;
        ...
        // 타겟 선정
        //int t = 0; <--- 위의 전역변수로 선언 변경 해놓고 여기도 선언한 거 안지워서 문제됨;;;
        t = 0;
        for (int i = 1; i < tList.size(); i++)
        {
        ...
        for (int i = 0; i < tList.size(); i++)
        {
            ...
            if (wayMap[r][c] != 1 && (r != tR || c != tC) && (r != aR || c != aC))
            {
                tList[i].p += 1;
                map[r][c] = tList[i].p;
            }
